 ...
 */
public class FuelSpeedoMeter extends Speedometer {

  private Path indicatorPath = new Path(), markPath = new Path();
  private Paint speedometerPaint = new Paint(Paint.ANTI_ALIAS_FLAG), pointerPaint = new Paint(Paint.ANTI_ALIAS_FLAG), pointerBackPaint =
      new Paint(Paint.ANTI_ALIAS_FLAG), indicatorPaint = new Paint(Paint.ANTI_ALIAS_FLAG), circlePaint = new Paint(Paint.ANTI_ALIAS_FLAG),
  ...
  public FuelSpeedoMeter(Context context) {
    super(context);
    init();
  }

  public FuelSpeedoMeter(Context context, AttributeSet attrs) {
    super(context, attrs);
    init();
    initAttributeSet(context, attrs);
  }

  public FuelSpeedoMeter(Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);
    init();
    initAttributeSet(context, attrs);
  }

  @Override protected void defaultValues() {
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    drawMinimumValue(canvas);
    drawMaximumValue(canvas);
    drawInitialValue(canvas);
  }

  private void drawInitialValue(Canvas canvas) {
    String speed = String.format(Locale.getDefault(), "%.1f", getCorrectSpeed());
    speedTextPaint.getTextBounds(speed, 0, speed.length(), bounds);
    canvas.drawText(speed, getWidth() / 2f - (bounds.width() / 2f), getHeight() * .9f, speedTextPaint);
    canvas.drawText(getUnit(), getWidth() / 2f + dpTOpx(1), getHeight() * .9f, unitTextPaint);
  }

  private void drawMaximumValue(Canvas canvas) {
    textPaint.setTextAlign(Paint.Align.RIGHT);
    canvas.drawText(String.format(Locale.getDefault(), "%d", getMaxSpeed()), getWidth() * 4 / 5f, getHeight() * 6 / 7f, textPaint);
  }

  private void drawMinimumValue(Canvas canvas) {
  ...
  private void drawIndicator(Canvas canvas) {
    ...
    canvas.drawCircle(getWidth() / 2f, getSpeedometerWidth() / 2f + dpTOpx(8), getSpeedometerWidth() / 2f + dpTOpx(8), pointerBackPaint);
    canvas.drawCircle(getWidth() / 2f, getSpeedometerWidth() / 2f + dpTOpx(8), getSpeedometerWidth() / 2f + dpTOpx(1), pointerPaint);
    canvas.drawPath(indicatorPath, indicatorPaint);
    canvas.restore();
  }

  private void drawOuterCircle(Canvas canvas) {
    canvas.drawArc(speedometerRect, MIN_DEGREE, 360f, false, speedometerPaint);
  }
  ...
  private void updateRadial() {
    ...
    RadialGradient pointerGradient =
        new RadialGradient(getWidth() / 2f, getSpeedometerWidth() / 2f + dpTOpx(8), getSpeedometerWidth() / 2f + dpTOpx(8),
            new int[] { centerColor, edgeColor }, new float[] { .4f, 1f }, Shader.TileMode.CLAMP);
  ...
  @Deprecated @Override public int getLowSpeedColor() {
    return super.getLowSpeedColor();
  }

  @Deprecated @Override public void setLowSpeedColor(int lowSpeedColor) {
  }

  @Deprecated @Override public int getMediumSpeedColor() {
    return super.getMediumSpeedColor();
  }

  @Deprecated @Override public void setMediumSpeedColor(int mediumSpeedColor) {
  }

  @Deprecated @Override public int getHighSpeedColor() {
    return super.getHighSpeedColor();
  }

  @Deprecated @Override public void setHighSpeedColor(int highSpeedColor) {
  }
}
